...
This list of packages that are installed here are not necessarily required for the keybindings to work, but are from packages which have required no config elsewhere but that I set the keybindings for here and thus need to be installed somewhere.

...
- [[#bibtex-completion-ivy-org-ref-and-org-cite][Bibtex completion (Ivy), Org-Ref and Org-Cite]]
  - [[#citar][Citar]]
  - [[#ebib][Ebib]]
...
- [[#export-a-bibtex-entry-to-org-roam][Export a bibtex entry to org-roam]]
...
Then, some helper functions related to dependent databases. I use this to help me organize my literature into smaller clusters, which is in some cases helpful.
...
In this section is all my config for improving the built-in reading list in ebib. It is generally a very nice interface, but I have added some more code so that it fits better in my config and interacts with org-roam. A brief description of everything in this section follows. 

First, I define the file in which the reading list is contained and the template with which each entry should be initialized. However, to more smoothly integrate with my org-roam ecosystem, the things this template gives you aren't enough. I want each entry to become an org-roam-node, link to the parent file (which is important so that the nodes don't appear as orphan nodes, which I try to avoid in my system) and get a priority (important to sort the reading list). This is the first 2 functions, which are added to the ~ebib-reading-list-new-item-hook~. Then, I define 3 predicates and the according filter functions. These are filter in and out ebib nodes with priority (returns the priority if it exists) and items with the "TO-READ" entry (usually only reading list items, but rarely I will also tag something else with this). Then, filter for to-read items, filter for everything besides reading list items (I use this as my base org-roam-node-find because the reading list adds a lot of noise having many nodes which are very often the last edited buffer) and filter for reading list items (this is an ~org-roam-node-read~ function because it is never used for directly finding the node).

The last 3 functions are utilizing this last filter to do changes to the reading list. The first function creates a reference note from a reading list item with the backend function ~orb--new-note~ as it can obtain the citekey easily. Then, there is a function to remove an entry from the reading list. This is done by setting it to DONE, which from a different part of my config ([[*Fleeting notes][Fleeting notes]]), already removes its ID. Lastly, a function to change the priority of a reading list item. The last two use the function ~evil-save-and-close~ because it's in my opinion easier to write it like this. It could in theory be replaced by sequentially running ~save-buffer~ and ~delete-window~ for vanilla Emacs functions.

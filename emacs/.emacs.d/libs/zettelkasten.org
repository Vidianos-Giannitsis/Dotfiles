...
* Org-roam-similarity
Org-similarity is a package to be used with org-roam for discovering documents similar to the current buffer. I discovered it from [[https://org-roam.discourse.group/t/a-demo-of-ai-for-linking-writing-and-thinking-with-org-roam-should-we-build-org-roam-ai/2891/3][this]] discussion in the Org-Roam Discourse group.

The original package has an input of a file and an output which is a list of links. I have decided to fork the project and widen its scope to be able to have an arbitrary org-roam file as its input and return a list of org-roam nodes. This section here is the changes I will make which are user customizations. The literate configuration for this package, which will be where all the code will be is going to be stored in ~/org-roam-similarity. 
...
#+RESULTS:
| *Similarity Results* | (display-buffer-in-side-window) | (inhibit-same-window . t) | (side . right)       | (window-width . 0.4)                   |
| \*org-roam\*       | (display-buffer-in-direction)   | (direction . right)       | (window-width . 0.4) | (window-height . fit-window-to-buffer) |
